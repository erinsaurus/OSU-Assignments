...
void getArrDep(State* currentState)
{
	for (int i = 0; i < 4; i++) //loop to calculate arrivals/departures at each light
	{
		currentState->nrOfVehiclesArrive(); //get arrivals for both directions
		currentState->nrOfVehiclesDepart(); //get departures for both directions
...
int main()
{
	...
	//link the objects together in a circle
	...
	for (int i = 0; i < 4; i++) //loop to display total output for arrivals/departures at each light
	{
		if (currentState->getDirection1() != "NoGreen" && currentState->getDirection2() != "NoGreen")
		{
			...
			std::cout << "Average wait time: "
				<< currentState->getWaitTime1() << " lights." << std::endl;
			std::cout << currentState->getDirection2()
				<< " lane: \n" << "Number of vehicles arrived: "
			...
			std::cout << "Average wait time: "
				<< currentState->getWaitTime2() << " lights." << std::endl;
		}
