...
const char *WINDOWTITLE = { "OpenCL/OpenGL Particle System -- Erin Alltop" };
...
const int NUM_PARTICLES = 8388608;
const int LOCAL_SIZE    = 64;
const char *CL_FILE_NAME   = { "alltope.cl" };
...
Display( )
{
	...
	{
		char str[128];
		sprintf( str, "Frame %8d -- %6.3f GigaParticles/Sec", Frame, (float)NUM_PARTICLES/ElapsedTime/1000000000. );
		printf("Frame %8d -- %6.3f GigaParticles/Sec\n", Frame, (float)NUM_PARTICLES / ElapsedTime / 1000000000.);
...
InitLists( )
{
	...
	// First sphere
	glutWireSphere(2400., 18., 9.);

	// Second sphere
	glPushMatrix();
	glTranslatef(0., -1300., 0.);
	glutWireSphere(1000., 60., 30.);
	glPopMatrix();
...
{
        char filename[128];
        sprintf( filename, "alltope.%03d.particles", Frame ); // use your engr login instead
