 ...
 ** CS475 - Spring 2018
 ** Project 2
 ** Using OpenMP with Coarse and Fine-Parallelism
 ** and Static and Dynamic Scheduling
 *********************************************/
...
int main(int argc, char * argv[]) 
{
 ...
 // fprintf(stderr, "Have %d processors.\n", numProcessors);
  ...
  for (int t = 0; t < NUMSTEPS; t++) 
  {
    ...
    // coarse-grained parallelism #pragma goes here
    // use schedule(static) or schedule(dynamic)

    #ifdef COARSE
    #ifdef STATIC
        //printf("Coarse static\n");
        #pragma omp parallel for default(none), shared(Bodies), schedule(static)
    #else
        //printf("Coarse dynamic\n");
    ...
    for (int i = 0; i < NUMBODIES; i++) 
    {
      ...
      // fine-grained parallelism #pragma goes here
      // fx, fy, fx need to undergo a reduction-add here
      // use schedule(static) or schedule(dynamic)

     #ifdef FINE
     #ifdef STATIC  
          //printf("Fine static\n");  
         #pragma omp parallel for default(none), reduction(+:fx, fy, fz), schedule(static), private(i), shared(bi, Bodies)
     #else
          //printf("Fine dynamic\n");
         #pragma omp parallel for default(none), reduction(+:fx, fy, fz), schedule(dynamic), private(i), shared(bi, Bodies)
     #endif
     #endif
