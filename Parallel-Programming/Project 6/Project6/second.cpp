...
int main( int argc, char *argv[ ] )
{
	...
	{
		hA[i] = hB[i] = hC[i] = (float) sqrt(  (double)i  );							// CHANGED
	}
	...
	if( status != CL_SUCCESS )
		...
		cl_mem dD = clCreateBuffer( context, CL_MEM_WRITE_ONLY, dataSize, NULL, &status );			//CHANGED
	if( status != CL_SUCCESS )
	...
	if( status != CL_SUCCESS )
		...
		status = clEnqueueWriteBuffer( cmdQueue, dC, CL_FALSE, 0, dataSize, hC, 0, NULL, NULL );				//CHANGED
	if( status != CL_SUCCESS )
	...
	status = clSetKernelArg( kernel, 3, sizeof(cl_mem), &dD );					// CHHANGED
	if( status != CL_SUCCESS )
...
/*
	...
	printf("%10.6f,", (double)NUM_ELEMENTS/(time1-time0)/1000000000. );
