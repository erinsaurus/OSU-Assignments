 ...
 ** This program is one of five. It performs
 ** exacly like otp_enc_d.c except it decrypts the
 ** given ciphertext using the passed-in ciphertext
 ** and key. It returns the plaintext message 
 ** to otp_dec.c
 **************************************************/
...
int main(int argc, char *argv[])
{
	...
	if (listenSocketFD < 0) { fprintf(stderr, "otp_dec_d error: problem opening socket\n"); exit(1); }

	// Enable the socket to begin listening
	if (bind(listenSocketFD, (struct sockaddr *)&serverAddress, sizeof(serverAddress)) < 0) // Connect socket to port
	{
		fprintf(stderr, "otp_dec_d error: problem with binding\n");
		exit(1);
	}
	listen(listenSocketFD, 5); // Flip the socket on - it can now receive up to 5 connections

	while(1) // loop children
	{
		...
		switch(pid)
		{
			...
			case 0:
				...
				if (charsRead < 0) 
				{
					fprintf(stderr, "otp_dec_d error: problem reading from socket\n");
					exit(1);
				}

				// check for special handshake 'code' for otp_dec. Anything else, exit with error
				if(strcmp(buffer, "###") != 0)
				{
				...
				{	// Get the message from the client
					...
					// Decrypting the message using ASCII characters
					int i;
					for(i = 0; i < msgLen; i++)
					{
						if(message[i] == 32) message[i] = 91;
						if(key[i] == 32) key[i] = 91;
						message[i] = (message[i] - 65 - (key[i] - 65)) % 27;
					...
					// Send decrypted code back to client
