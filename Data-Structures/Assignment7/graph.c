...
{// FIXME: Implement
    ...
    //clear visited nodes of the graph
    clearVisited(graph);
   
    //if destination is already ready, return  true - O(1)!
    if(dfsRecursive(graph, source, destination)){
        return 1;
    }
    
     //using a stack for this function - push back / pop back
    //push the source to the bottom of the stack
    Deque* stack = dequeNew();
    dequePushBack(stack, source);
    
    //added variable to store return to prevent memory leaks
    int path = 0;
    
    //loop through vertices starting at source
    //while loop - pop off the top of the stack, mark node as visited
    while(!dfsRecursive(graph, source, destination) && !dequeIsEmpty(stack)){
        //set iterator equal to back of the stack and set visited to true
        Vertex* currVer = dequeBack(stack);
        currVer->isVisited = 1;
        dequePopBack(stack); //pop off the back of the stack
        
        //if equal to destination, set path to true
        if(currVer == destination){
            path = 1;
        }
        
        //for the number of neighbors, if the neighbor has not been visited, push it to the back of the stack
        for(int i = 0; i < currVer->numNeighbors; i++){
            Vertex* neighbor = currVer->neighbors[i];

            if(!neighbor->isVisited){
                dequePushBack(stack, neighbor);
            }
        }
    }
    
    //push neighbors onto top of the stack if they are not already visited/reachable
    
    //if you reach a dead end, go back to last decision made
...
{// FIXME: Implement
    ...
    //uses a queue - push back / pop front 
    ...
    //while there is no destination and queue is not empty
    while(!dfsRecursive(graph, source, destination) && !dequeIsEmpty(queue)){
        //set iterator to front of queue and set to visited. Remove front of queue
        Vertex* currVer = dequeFront(queue);
        currVer->isVisited = 1;
        dequePopFront(queue);
        
        //if destination found, set path to true
        if(currVer == destination){
            path = 1;
        }
        
        //for each neighbor of the current index, if they are not visited push them to the queue
        for(int i = 0; i < currVer->numNeighbors; i++){
            Vertex* neighbor = currVer->neighbors[i];

            if(!neighbor->isVisited){
               dequePushBack(queue, neighbor);      
            } 
        }
    }
    
    dequeDelete(queue); //delete the queue
    return path; //return true or false
}
